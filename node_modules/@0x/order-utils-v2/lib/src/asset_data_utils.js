...
exports.assetDataUtils = {
    ...
    assertIsERC20AssetData: function (assetData) {
        ...
        exports.assetDataUtils.assertWordAlignedAssetData(assetData);
        var assetProxyId = exports.assetDataUtils.decodeAssetProxyId(assetData);
        if (assetProxyId !== types_1.AssetProxyId.ERC20) {
    ...
    assertIsERC1155AssetData: function (assetData) {
        if (assetData.length < constants_1.constants.ERC1155_ASSET_DATA_MIN_CHAR_LENGTH_WITH_PREFIX) {
            throw new Error("Could not decode ERC1155 Proxy Data. Expected length of encoded data to be at least " + constants_1.constants.ERC1155_ASSET_DATA_MIN_CHAR_LENGTH_WITH_PREFIX + ". Got " + assetData.length);
        }
        exports.assetDataUtils.assertWordAlignedAssetData(assetData);
        var assetProxyId = exports.assetDataUtils.decodeAssetProxyId(assetData);
        if (assetProxyId !== types_1.AssetProxyId.ERC1155) {
            throw new Error("Could not decode ERC1155 assetData. Expected assetProxyId to be ERC1155 (" + types_1.AssetProxyId.ERC1155 + "), but got " + assetProxyId);
        }
    },
    /**
    ...
    assertIsStaticCallAssetData: function (assetData) {
        if (assetData.length < constants_1.constants.STATIC_CALL_ASSET_DATA_MIN_CHAR_LENGTH_WITH_PREFIX) {
            throw new Error("Could not decode StaticCall Proxy Data. Expected length of encoded data to be at least " + constants_1.constants.STATIC_CALL_ASSET_DATA_MIN_CHAR_LENGTH_WITH_PREFIX + ". Got " + assetData.length);
        }
        exports.assetDataUtils.assertWordAlignedAssetData(assetData);
        var assetProxyId = exports.assetDataUtils.decodeAssetProxyId(assetData);
        if (assetProxyId !== types_1.AssetProxyId.StaticCall) {
            throw new Error("Could not decode StaticCall assetData. Expected assetProxyId to be StaticCall (" + types_1.AssetProxyId.StaticCall + "), but got " + assetProxyId);
        }
    },
    /**
     ...
     * @param assetData Hex encoded assetData string
     */
    assertWordAlignedAssetData: function (assetData) {
        var charsIn32Bytes = 64;
        if ((assetData.length - constants_1.constants.SELECTOR_CHAR_LENGTH_WITH_PREFIX) % charsIn32Bytes !== 0) {
            throw new Error("assetData must be word aligned. " + (assetData.length - 2) / 2 + " is not a valid byte length.");
        }
    },
    /**
    ...
    decodeAssetDataOrThrow: function (assetData) {
        ...
        switch (assetProxyId) {
            ...
            case types_1.AssetProxyId.StaticCall:
                var staticCallData = exports.assetDataUtils.decodeStaticCallAssetData(assetData);
