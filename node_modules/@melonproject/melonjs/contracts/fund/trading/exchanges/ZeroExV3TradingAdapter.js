...
const ZeroExV2TradingAdapter_1 = require("./ZeroExV2TradingAdapter");
class ZeroExV3TradingAdapter extends BaseTradingAdapter_1.BaseTradingAdapter {
    getOrderHash(order) {
        return order_utils_1.orderHashUtils.getOrderHashAsync(order);
    }
    /**
    ...
    makeOrder(from, order) {
        ...
        const takerTokenAddress = order_utils_1.assetDataUtils.decodeERC20AssetData(order.takerAssetData).tokenAddress;
    ...
    takeOrder(from, order, takerAmount) {
        const makerTokenAddress = order_utils_1.assetDataUtils.decodeERC20AssetData(order.makerAssetData).tokenAddress;
        const takerTokenAddress = order_utils_1.assetDataUtils.decodeERC20AssetData(order.takerAssetData).tokenAddress;
        const makerFeeAsset = order.makerFeeAssetData === '0x'
            ? zeroAddress_1.zeroAddress
            : order_utils_1.assetDataUtils.decodeERC20AssetData(order.makerFeeAssetData).tokenAddress;
        const takerFeeAsset = order.takerFeeAssetData === '0x'
            ? zeroAddress_1.zeroAddress
            : order_utils_1.assetDataUtils.decodeERC20AssetData(order.takerFeeAssetData).tokenAddress;
    ...
    cancelOrder(from, args) {
        ...
        const validate = () => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!orderHashHex) {
                throw new ZeroExV2TradingAdapter_1.MissingZeroExOrderHashHex();
            }
        });
        return this.trading.callOnExchange(from, methodArgs, validate);
    }
    createUnsignedOrder(values) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const makerFeeAssetData = values.makerFeeTokenAddress
                ? order_utils_1.assetDataUtils.encodeERC20AssetData(values.makerFeeTokenAddress)
                : zeroAddress_1.zeroAddress;
            const takerFeeAssetData = values.takerFeeTokenAddress
                ? order_utils_1.assetDataUtils.encodeERC20AssetData(values.takerFeeTokenAddress)
                : zeroAddress_1.zeroAddress;
            const order = {
                exchangeAddress: this.info.exchange,
                chainId: yield this.environment.client.net.getId(),
                ...
                makerAssetData: order_utils_1.assetDataUtils.encodeERC20AssetData(values.makerTokenAddress),
                takerAssetData: order_utils_1.assetDataUtils.encodeERC20AssetData(values.takerTokenAddress),
                makerFee: values.takerFee || zeroBigNumber_1.zeroBigNumber,
    ...
    signOrder(order, signer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const signed = yield order_utils_1.signatureUtils.ecSignOrderAsync(provider, order, signer);
            if (sameAddress_1.sameAddress(signed.makerAddress, signer)) {
                return signed;
            }
            return Object.assign(Object.assign({}, signed), { signature: `${signed.signature.slice(0, -1)}${order_utils_1.SignatureType.PreSigned}` });
        });
    }
    getSubprovider() {
...
exports.ZeroExV3TradingAdapter = ZeroExV3TradingAdapter;
